...
export function NotFoundExperience({
  ...
  const helperCards = [
    {
      key: "finder",
      title: copy.helperCards.finder.title,
      description: copy.helperCards.finder.description,
      Icon: DiscoverIcon,
    },
    {
      key: "catalog",
      title: copy.helperCards.catalog.title,
      description: copy.helperCards.catalog.description,
      Icon: BuyIcon,
    },
    {
      key: "compare",
      title: copy.helperCards.compare.title,
      description: copy.helperCards.compare.description,
      Icon: CompareIcon,
    },
  ];
      ...
      <section className="paper-texture relative overflow-hidden rounded-[2.2rem] border border-[#e4d8c8] bg-[#f8f3ea] px-6 py-9 shadow-[0_30px_90px_-55px_rgba(50,35,20,0.52)] sm:px-10 sm:py-12 lg:px-14 lg:py-16">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <Link href={primaryHref} className={buttonStyles({ size: "lg", className: "min-w-[11rem]" })}>
              {copy.primaryCta}
            </Link>
            <Link
              href={secondaryHref}
      ...
      {alternatives.length > 0 ? (
        <section className="mt-24 space-y-8">
          <SectionTitle
            ...
            subtitle={copy.alternatives.subtitle}
          />
